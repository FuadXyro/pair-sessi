...
async function handler(m, {
    ...
    const count = args[0]
    if (!count) return m.reply("( ! ) Masukkan angka jumlah sumbangan.")
    if (isNaN(count)) return m.reply("( ! )Jumlah sumbangan harus berupa angka.")
...
handler.before = async m => {
    ...
    await m.reply("[ ✘ ] Tidak bisa meminta sumbangan ke diri anda sendiri!.")
    }
    }
