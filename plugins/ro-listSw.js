...
conn.storyData = conn.storyData ? conn.storyData : {};
    ...
    if (list.length === 0) {
        return await conn.reply(m.chat, `Tidak ada story yang tersedia saat ini. Silakan tambahkan cerita dengan mengirim gambar, video, atau pesan suara.`, m, { mentions: [m.sender] });
    }
...
if (m.isBaileys || !(m.chat in conn.storyData)) return;
  ...
  await conn.reply(m.chat, "( ✘ ) Harap Masukkan nomor video yang valid.", m);
  } else {
