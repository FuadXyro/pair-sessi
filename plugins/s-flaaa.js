...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  if (command == "flatheme") {
    ...
    var listnya = listSections.map((v, index) => {
      return `[ ${++index} ] ${v.split(/[\s/?]+/)[2]}`;
    }).join('\n');
    await m.reply("*L I S T - T E M A*\nContoh:\n.sfla Design-Fire|hello guys\n\n" + listnya);
  }

  if (command == "sfla") {
    if (!one) throw "input design\nContoh:\n.sfla Design-Fire|hello guys\nketik *.flatheme* query|page untuk mencari list tema";
...
handler.command = /^(sfla|flatheme)$/i;
